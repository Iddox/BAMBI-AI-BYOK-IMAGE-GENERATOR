...
/**
 * API pour générer des images avec xAI en utilisant la même logique que la page de test
 * Cette route est spécifiquement conçue pour résoudre les problèmes d'encodage avec l'API xAI
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (configError) {
      console.error('xai-generate - Erreur lors de la récupération de la configuration:', configError);
    ...
    try {
      ...
      if (!apiKey) {
        console.error('xai-generate - Clé API vide après déchiffrement');
        return NextResponse.json({ error: 'Erreur de déchiffrement de la clé API' }, { status: 400 });
      }
    } catch (decryptError) {
      console.error('xai-generate - Erreur lors du déchiffrement de la clé API:', decryptError);
      return NextResponse.json({ error: 'Erreur de déchiffrement de la clé API' }, { status: 400 });
    }
    ...
    console.log('xai-generate - Requête à l\'API xAI:', JSON.stringify(xaiRequestBody));
    console.log('xai-generate - Longueur de la clé API:', cleanApiKey.length);
    console.log('xai-generate - Premiers caractères de la clé API:', cleanApiKey.substring(0, 5) + '...');

    try {
      // Afficher plus d'informations sur la clé API pour le débogage
      console.log('xai-generate - Clé API masquée:', cleanApiKey.substring(0, 5) + '...' + cleanApiKey.substring(cleanApiKey.length - 5));
      console.log('xai-generate - Format de la clé API valide:', /^[A-Za-z0-9\-_]+$/.test(cleanApiKey));

      // Appel à l'API xAI sans timeout côté serveur (le timeout est géré par le client)
      ...
      try {
        ...
        console.error('xai-generate - Erreur lors de l\'analyse de la réponse:', parseError);
        console.error('xai-generate - Réponse brute:', responseText);
        return NextResponse.json({ error: 'Erreur lors de l\'analyse de la réponse de l\'API xAI' }, { status: 500 });
      }

      // Vérifier si la réponse contient une erreur
      if (!xaiResponse.ok) {
        console.error('xai-generate - Erreur de l\'API xAI:', responseData);
        console.error('xai-generate - Statut de la réponse:', xaiResponse.status, xaiResponse.statusText);
        console.error('xai-generate - Texte brut de la réponse:', responseText);
      ...
      // Simplification temporaire : ne pas enregistrer les images dans la base de données
      // et retourner directement les URLs des images
      console.log('xai-generate - Images générées avec succès:', imageUrls.length);
      ...
      // Retourner les images générées sans les enregistrer dans la base de données
      return NextResponse.json({
        images: generatedImages
      });
    } catch (fetchError) {
      console.error('xai-generate - Erreur lors de la communication avec l\'API xAI:', fetchError);

      // Vérifier si c'est une erreur d'abandon (timeout)
      if (fetchError.name === 'AbortError') {
        return NextResponse.json({
          error: `Délai d'attente dépassé lors de la communication avec l'API xAI`,
          details: {
            message: "La requête a été abandonnée car elle a pris trop de temps",
            type: "timeout_error",
            retryable: true
          }
      ...
      return NextResponse.json({
        error: `Erreur lors de la communication avec l'API xAI: ${fetchError.message}`,
        details: { message: fetchError.message, stack: fetchError.stack }
      }, { status: 500 });
    }
  } catch (error: any) {
    console.error('xai-generate - Erreur:', error);
